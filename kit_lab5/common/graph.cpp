...
void store_solution_binary(const char *filename, Graph graph, double *solution)
{
    ...
    if (fwrite(solution, sizeof(double), graph->num_nodes, output) != (size_t)graph->num_nodes)
    {
        fprintf(stderr, "Error writing solution.\n");
        exit(1);
    }
...
void load_solution_binary(const char *filename, double *solution)
{
    // double* solution;
    ...
    int num_nodes = header[1];

    // solution = (double*)malloc(sizeof(double) * num_nodes);

    if (fread(solution, sizeof(double), num_nodes, input) != (size_t)num_nodes)
    {
        fprintf(stderr, "Error reading nodes.\n");
        exit(1);
    }
...
void store_solution_binary_int(const char *filename, Graph graph, int *solution)
{
    ...
    if (fwrite(solution, sizeof(int), graph->num_nodes, output) != (size_t)graph->num_nodes)
    {
        fprintf(stderr, "Error writing solution.\n");
        exit(1);
    }
...
void load_solution_binary_int(const char *filename, int *solution)
{
    ...
    int num_nodes = header[1];
    if (fread(solution, sizeof(int), num_nodes, input) != (size_t)num_nodes)
    {
        fprintf(stderr, "Error reading nodes.\n");
        exit(1);
    }
